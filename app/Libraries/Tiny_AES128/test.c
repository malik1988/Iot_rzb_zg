...
void Encrypt(uint8_t* src, uint8_t* out) {
	uint8_t cKey[16] = "fssfwefwefijgljlkghkjklj";
	AES128_ECB_encrypt(src, cKey, out);
}

void Decrypt(uint8_t* src, uint8_t* out) {
	uint8_t cKey[16] = "fssfwefwefijgljlkghkjklj";
	AES128_ECB_decrypt(src, cKey, out);
}

void test_str() {
	uint8_t cKey[16] = "1234567afed23456";
	uint8_t cIv[] = "1234567afed23456";
	uint8_t cSrc[] = "1234567afed23456";
	uint8_t buf[16];
	//ECB以128位为单位进行单独加密或者解密
	//AES128_ECB_encrypt该函数每次只能加密128位数据，如果需要大数据块加密需要循环调用
	AES128_ECB_encrypt(cSrc, cKey, buf);



	AES128_ECB_decrypt(buf, cKey, buf);

	printf("ECB 解密数据:%s", buf);
}

int test_AES128_main(void) {
...
static void test_decrypt_cbc(void) {
	...
	while (retry--) {
		...
		delta = us_now - us_pre;
		avg += delta;
		printf("%s,delta %d,avg %d\n", __func__, delta, avg / (20 - retry));
	}
